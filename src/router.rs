...
pub fn build_router(state: AppState) -> Router {
    // --- Proxy API routes (client-key Bearer auth + rate-limit) ---
    ...
    let admin_routes = Router::new()
        ...
        .route("/admin/client-keys",
            get(handlers::admin::get_client_keys)
            .post(handlers::admin::add_client_key))
        .route("/admin/client-keys/{key}",
            delete(handlers::admin::delete_client_key))
        .route("/admin/downstream-keys",
            post(handlers::admin::add_downstream_key))
        .route("/admin/downstream-keys/{key}",
            patch(handlers::admin::update_downstream_key)
            .delete(handlers::admin::delete_downstream_key))
        .route_layer(middleware::from_fn_with_state(
            state.clone(),
            basic_auth,
        ));
